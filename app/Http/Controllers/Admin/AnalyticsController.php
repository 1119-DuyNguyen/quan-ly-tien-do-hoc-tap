...
class AnalyticsController extends ApiController
{
    ...
    public function index(PaginationRequest $request)
    {
        ...
        $tong_so_sv = dssvTable()
            ->get()
            ->count();
        $so_sv_tot_nghiep = dssvTable()
            ->where('da_tot_nghiep', '=', '1')
            ->get()
            ->count();
        $so_sv_chua_tot_nghiep = $tong_so_sv - $so_sv_tot_nghiep;

        $so_sv_bi_canh_cao = dssvTable()
            ->where('so_lan_canh_cao', '>', '0')
            ->get()
            ->count();
        $so_sv_bi_bth = dssvTable()
            ->where('buoc_thoi_hoc', '=', '1')
            ->get()
            ->count();
        ...
        if ($request->input('nganh') !== null) {
            ...
            $object->tong_sv = dssvTable()
                ->where('nganh_id', '=', $nganh_id)
                ->get()
                ->count();
            $object->sv_tot_nghiep = dssvTable()
                ->where('nganh_id', '=', $nganh_id)
                ->where('da_tot_nghiep', '=', '1')
                ->get()
                ->count();
        ...
        } elseif ($request->input('khoa') !== null) {
